...
##############################################
# Policy for Backend S3 and Dynamo DB access #
##############################################
data "aws_iam_policy_document" "tf_backend" {
  statement {
    effect    = "Allow"
    actions   = ["s3:ListBucket"] # Fixed typo here
  ...
  statement {
    ...
    resources = [ # Changed from string to list
...
resource "aws_iam_policy" "tf_backend" {
  ...
  description = "Allow user to use S3 and DynamoDB for TF resources"
